...
else
{
	...
	{	
		...
		//stream_socket_accept($socket1,-1)
...
function do_listen($socket1)
{
	$socket_pi_status = "waiting_for_connection";
	echo $socket_pi_status."\n";
	...
	while ($conn1 = @stream_socket_accept($socket1,-1))
	{
		...
		//{
			...
			$rt=@fwrite($conn1, 'Socket-1 on Port 8001 : Connection from RaspberryPi Established' . "\n");
			//echo $rt."\n";
...
*/
			...
			while(1)
			{
				if(check_connection_alive($conn1)=="connection_lost")
				{
					return;
				}
			}
...
function check_connection_alive($conn1)
{
	$socket_pi_status = "connection_established";
	
	$rt=@fwrite($conn1, 'Socket-1 on Port 8001 : Check Connection Alive' . "\n");
	//echo $rt."\n";
	
	if($rt<=0)
	{
		$socket_pi_status = "connection_lost";
		echo "Socket-1 on Port 8001 : Connection from RaspberryPi Lost-".$socket_pi_status. "\n";	
		fclose($conn1);
		return $socket_pi_status;
	}
